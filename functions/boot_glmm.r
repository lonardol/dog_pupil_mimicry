...
boot.lmer<-function(m, discard.warnings=F, nboots=1000, para=F, resol=1000, level=0.95,
  ...
  library(kyotil, lib.loc=lib.loc)
  n.cores=n.cores[1]
  if(!is.null(use)){
  ...
  if(para){
    ...
    if(is.numeric(n.cores)){
      n.cores=min(c(n.cores, getOption("cl.cores", detectCores())))
    }else{
      n.cores=getOption("cl.cores", detectCores())
    }
    cl <- makeCluster(spec=n.cores)
...
boot.glmmtmb<-function(m, data, discard.non.conv=F, nboots=1000, para=F, resol=100, level=0.95,
                       use=NULL, circ.var.name=NULL, circ.var=NULL, transf.beta=F, 
  ...
  boot.fun<-function(x, xcall., data., rv.name., m., discard.non.conv., save.path., extract.all., beta.tr.){
    xdone=F
    while(!xdone){
      done2=F
      while(!done2){
        ...
        if(xcall[["xfam"]][["family"]]!="beta"){
          ...
          if(transf.beta){
            data.[, rv.name.]=beta.tr.(data.[, rv.name.])
            done2=T
          }else if(min(data.[, rv.name.])>0 & max(data.[, rv.name.])<1){
            done2=T
          }
        }
      }
  ...
  if(para){
    ...
    all.res=parLapply(cl=cl, X=1:nboots, fun=boot.fun, m.=m, xcall.=xcall, data.=data, rv.name.=rv.name, 
                      discard.non.conv.=discard.non.conv, save.path.=save.path, extract.all.=extract.all, beta.tr.=beta.tr)
